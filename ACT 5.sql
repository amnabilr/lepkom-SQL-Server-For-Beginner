...
Select LOWER (Ename) as Nama, LEN (Ename) as Panjang From EMP Where DEPTNO = 10

Select LEFT (Ename, 3) as Nickname from EMP where JOB = 'Salesman'

Select RIGHT (Ename, 3) as LastName from EMP where MGR = 7839
Select ABS ([SAL] + (SAL * 0.1)) as Total from EMP 

Select DATEPART ("dy", Getdate () ) as Today_Is

Select REVERSE (Ename) as Nama_Kebalikan, LOWER (JOB) as Pekerjaan, SAL from EMP 